...
# dbLoadTemplate("$(TOP)/galil_DMCApp/Db/$(PORT)_motors-v6-9down.substitutions", "P=$(P), PORT=$(PORT)")
#Motor record version 6-10 and up
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/$(PORT)_motors-v6-10up.substitutions", "P=$(P), PORT=$(PORT)")

#Load DMC controller features (eg.  Limit switch type, home switch type, output compare, message consoles)
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_dmc_ctrl.substitutions", "P=$(P), PORT=$(PORT)")

#Load extra features for real axis/motors (eg. Motor type, encoder type)
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/$(PORT)_motor_extras.substitutions", "P=$(P), PORT=$(PORT)")

#Load extra features for CS axis/motors (eg. Setpoint monitor)
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_csmotor_extras.substitutions", "P=$(P), PORT=$(PORT)")

#Load kinematics for CS axis/motors (eg. Forward and reverse kinematics, kinematic variables)
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_csmotor_kinematics.substitutions", "P=$(P), PORT=$(PORT)")

#Load coordinate system features (eg. Coordinate system S and T status, motor list, segments processed, moving status)
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_coordinate_systems.substitutions", "P=$(P), PORT=$(PORT)")

#Load digital IO databases
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_dmc_digital_ports.substitutions", "P=$(P), PORT=$(PORT)")

#Load analog IO databases
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_dmc_analog_ports.substitutions", "P=$(P), PORT=$(PORT)")

#Load user defined functions
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_userdef_records.substitutions", "PORT=$(PORT)")

#Load user defined array support
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_user_array.substitutions", "P=$(P), PORT=$(PORT)")

#Load profiles
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/$(PORT)_profileMoveController.substitutions", "P=$(P), PORT=$(PORT)")
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/$(PORT)_profileMoveAxis.substitutions", "P=$(P), PORT=$(PORT)")
