...
#epicsEnvSet("GALIL_DEBUG_FILE", "galil_debug1.txt")

# Asyn port name (eg. DMC01, DMC02, RIO01)
epicsEnvSet("PORT1", "18ID_DMC_E06")

# Controller address (IP address, serial port)
epicsEnvSet("ADDRESS1", "10.10.10.8")

# Controller update period Unit = millisecond
# Range 2-200
epicsEnvSet("UPDPERIOD1", "8")

##################################################################################################
# Derived configuration settings

# Record prefix derived from asyn port name
epicsEnvSet("P1", "$(PORT1):")

##################################################################################################

#Load motor records for real and coordinate system (CS) motors
#Motor record version 6-9 and below
# dbLoadTemplate("$(TOP)/galil_DMCApp/Db/$(PORT1)_motors-v6-9down.substitutions", "P=$(P1), PORT=$(PORT1)")
#Motor record version 6-10 and up
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/$(PORT1)_motors-v6-10up.substitutions", "P=$(P1), PORT=$(PORT1)")

#Load DMC controller features (eg.  Limit switch type, home switch type, output compare, message consoles)
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_dmc_ctrl.substitutions", "P=$(P1), PORT=$(PORT1)")

#Load extra features for real axis/motors (eg. Motor type, encoder type)
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/$(PORT1)_motor_extras.substitutions", "P=$(P1), PORT=$(PORT1)")

#Load extra features for CS axis/motors (eg. Setpoint monitor)
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_csmotor_extras.substitutions", "P=$(P1), PORT=$(PORT1)")

#Load kinematics for CS axis/motors (eg. Forward and reverse kinematics, kinematic variables)
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_csmotor_kinematics.substitutions", "P=$(P1), PORT=$(PORT1)")

#Load coordinate system features (eg. Coordinate system S and T status, motor list, segments processed, moving status)
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_coordinate_systems.substitutions", "P=$(P1), PORT=$(PORT1)")

#Load digital IO databases
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_dmc_digital_ports.substitutions", "P=$(P1), PORT=$(PORT1)")

#Load analog IO databases
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_dmc_analog_ports.substitutions", "P=$(P1), PORT=$(PORT1)")

#Load user defined functions
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_userdef_records.substitutions", "PORT=$(PORT1)")

#Load user defined array support
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/galil_user_array.substitutions", "P=$(P1), PORT=$(PORT1)")

#Load profiles
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/$(PORT1)_profileMoveController.substitutions", "P=$(P1), PORT=$(PORT1)")
dbLoadTemplate("$(TOP)/galil_DMCApp/Db/$(PORT1)_profileMoveAxis.substitutions", "P=$(P1), PORT=$(PORT1)")
...
GalilCreateController("$(PORT1)", "$(ADDRESS1)", "$(UPDPERIOD1)")
...
# Create the axis
GalilCreateAxis("$(PORT1)","A","",1)
GalilCreateAxis("$(PORT1)","B","",1)
GalilCreateAxis("$(PORT1)","C","",1)
GalilCreateAxis("$(PORT1)","D","",1)
GalilCreateAxis("$(PORT1)","E","",1)
GalilCreateAxis("$(PORT1)","F","",1)
GalilCreateAxis("$(PORT1)","G","",1)
GalilCreateAxis("$(PORT1)","H","",1)
...
# GalilAddCode("$(PORT1)", 1, "customcode.dmc")
...
# 2. char *Axis A-H
# 3. char *code_file custom code file
# GalilReplaceHomeCode("$(PORT1)", "A", "homeA.dmc")
...
GalilStartController("$(PORT1)", "", 1, 0)

# Start the controller
# Example using homing routine template assembly
#GalilStartController("$(PORT1)", "$(GALIL)/GalilSup/Db/galil_Default_Header.dmc;$(GALIL)/GalilSup/Db/galil_Home_RevLimit.dmc!$(GALIL)/GalilSup/Db/galil_Home_ForwLimit.dmc!$(GALIL)/GalilSup/Db/galil_Home_Home.dmc!$(GALIL)/GalilSup/Db/galil_Home_ForwLimit.dmc!$(GALIL)/GalilSup/Db/galil_Piezo_Home.dmc!$(GALIL)/GalilSup/Db/galil_Piezo_Home.dmc!$(GALIL)/GalilSup/Db/galil_Piezo_Home.dmc!$(GALIL)/GalilSup/Db/galil_Piezo_Home.dmc;$(GALIL)/GalilSup/Db/galil_Default_Footer.dmc", 0, 0, 3)
