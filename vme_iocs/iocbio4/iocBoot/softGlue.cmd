...
#initIP_EP200_FPGA(0, 1, "$(SOFTGLUE)/softGlueApp/Db/SoftGlue_2_2_Octupole_0_0.hex")
#    standard softGlue 2.2 plus two up/down counters plus two quadrature decoders
initIP_EP200_FPGA(0, 1, "$(SOFTGLUE)/softGlueApp/Db/SoftGlue_2_2_Encoder.hex")
...
initIP_EP200(0, 1, "SGIO_1", "SGIO_2", "SGIO_3", 0x800000)
...
initIP_EP200_Int(0, 1, 0x90, 0x0, 0x0, 0x0, 0x0)
...
#initIP_EP200_IO(0, 1, 201, 0x3c)
initIP_EP200_IO(0, 1, 201, 0xc)
#initIP_EP200_IO(0, 1, 202, 0x13)
...
dbLoadRecords("$(SOFTGLUE)/db/softGlue_SignalShow.db","P=18ID:,H=SG:,PORT=SOFTGLUE,READEVENT=20")
...
dbLoadRecords("$(SOFTGLUE)/db/softGlue_FPGAContent.db", "P=18ID:,H=SG:,PORT=SOFTGLUE,READEVENT=20")

# This database contains the additional records needed for
# SoftGlue_2_2_Octupole_0_0.hex
#!dbLoadRecords("$(SOFTGLUE)/db/softGlue_FPGAContent_octupole.db", "P=18ID:,H=SG:,PORT=SOFTGLUE,READEVENT=20")

# This database contains the additional records needed for
# SoftGlue_2_2_Encoder.hex
dbLoadRecords("$(SOFTGLUE)/db/softGlue_FPGAContent_Encoder.db", "P=18ID:,H=SG:,PORT=SOFTGLUE,READEVENT=20")

#    Interrupt support.
#    ('putenv' is used to fit the command into the vxWorks command line space.)
putenv "SFDB=$(SOFTGLUE)/db/softGlue_FPGAInt.db"
dbLoadRecords("$(SFDB)","P=18ID:,H=SG:,PORT1=SGIO_1,PORT2=SGIO_2,PORT3=SGIO_3,FIFO=10")

taskDelay(50)

#    Some stuff just for convenience: software clock and pulse generators, and
#    a couple of busy records.
dbLoadRecords("$(SOFTGLUE)/db/softGlue_convenience.db", "P=18ID:,H=SG:")

# Menu of softGlue circuits
# configMenu example.  See create_manual_set() command after iocInit.
dbLoadRecords("$(AUTOSAVE)/asApp/Db/configMenu.db","P=18ID:,CONFIG=SG")
...
doAfterIocInit("create_manual_set('SGMenu.req','P=18ID:,CONFIG=SG,H=SG:,CONFIGMENU=1')")
...
#sampleCustomInterruptPrepare(0, 1, 0x800010, 0x1)
